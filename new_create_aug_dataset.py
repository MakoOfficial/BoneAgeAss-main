...
# aug_dataset = torchvision.transforms.Compose([
#     torchvision.transforms.RandomResizedCrop(512, scale=(0.5, 1.0), ratio=(3.0/4.0, 4.0/3.0)),
#     torchvision.transforms.RandomAffine(degrees=20, translate=0.2, scale=0.2, fill=0.0),
...
def creatSmallDataset(data_dir, target_size, DF):
    """创建新的数据集"""
    label_list = [['id', 'boneage', 'male']]
    ...
    for idx, row in DF.iterrows():
        ...
        if label not in label_count or label_count[label] < target_size:
            copyfile(filename, os.path.join(data_dir, 'small-dataset', label))
            label_list.append([row['id'], row['boneage'], row['male']])
...
def reorg_SmallDataset(data_dir, target_size, DF):
    """将验证集和数据集分开"""
...
def reorg_aug_data(data_dir, target_size, valid_size):
    ...
    creatSmallDataset(data_dir, target_size, df)
    aug_all(data_dir, 'small-dataset', aug_dataset, target_size, labels)
    ...
    reorg_SmallDataset(data_dir, valid_size, new_df)
...
if __name__ == '__main__':
    ...
    valid_size = 5
    # move_all_files(os.path.join(data_dir, 'small-dataset'), os.path.join(data_dir, 'small-dataset'))
    reorg_aug_data(data_dir=data_dir, target_size=target_size, valid_size=valid_size)